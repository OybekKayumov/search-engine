...
public class SearchServiceImpl implements SearchService {
  ...
  public List<SearchDto> allSiteSearch(String searchText, int offset, int limit) {
	  ...
	  List<LemmaEntity> foundLemmaList = new ArrayList<>();
		...
		for (LemmaEntity l : foundLemmaList) {
		  if (l.getLemma().equals(searchText)) {
			  searchData = new ArrayList<>(getSearchDtoList(foundLemmaList,
								 textLemmaList, offset, limit));
				searchData.sort((o1, o2) -> Float.compare(o2.getRelevance(),
								 o1.getRelevance()));
	...
	private List<SearchDto> getSearchData(Hashtable<PageEntity, Float> pageList
		...
		for (PageEntity page : pageList.keySet()) {
			...
			result.add(new SearchDto(site, siteName, uri, title, snippet,
							absRelevance));
		}
	...
	private List<String> getWordsFromContent(String content,
	                                         List<Integer> lemmaIndex) {
	...
	private List<SearchDto> getSearchDtoList(List<LemmaEntity> lemmaList,
	                                         List<String> textLemmaList, int offset, int limit) {

		List<SearchDto> result = new ArrayList<>();
		pageRepository.flush();
		if (lemmaList.size() >= textLemmaList.size()) {
			List<PageEntity> foundPageList =
							pageRepository.findByLemmaList(lemmaList);
			indexRepository.flush();
			List<IndexEntity> foundIndexList =
							indexRepository.findByPagesAndLemmas(lemmaList, foundPageList);
			Hashtable<PageEntity, Float> sortedPageByAbsRelevance =
							getPageAbsRelevance(foundPageList, foundIndexList);
	...
	private Hashtable<PageEntity, Float> getPageAbsRelevance(List<PageEntity> pageList, List<IndexEntity> indexList) {
		...
		for (PageEntity page : pageWithRelevance.keySet()) {
			float absRelevance =
							pageWithRelevance.get(page) / Collections.max(pageWithRelevance.values());
