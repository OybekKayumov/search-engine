    ...
    private List<PageDto> getPageDtoList() throws InterruptedException {
        if (!Thread.interrupted()) {
            ...
            ForkJoinPool forkJoinPool =
                    new ForkJoinPool(processorCoreCount);
            List<PageDto> pages =
                    forkJoinPool.invoke(new PageUrlParser(urlFormat,
                            pageDtoVector, urlList));
    ...
    private void getLemmasPage() {
        ...
        if (!Thread.interrupted()) {
            ...
            for (LemmaDto lemmaDto : lemmaDtoList) {
                lemmaList.add(new LemmaEntity(lemmaDto.getLemma(),
                        lemmaDto.getFrequency(), siteEntity));
            }
            lemmasRepository.flush();
            lemmasRepository.saveAll(lemmaList);
        } else {
    ...
    private void indexingWords() throws InterruptedException {
        if (!Thread.interrupted()) {
            ...
            List<IndexDto> indexDtoList =
                    new CopyOnWriteArrayList<>(indexParser.getIndexList());
            ...
            for (IndexDto indexDto : indexDtoList) {
                ...
                LemmaEntity lemma = lemmasRepository.getById(indexDto.getLemmaId());
